...
class MY_Loader extends MX_Loader
{
  ...
  function _zen_show_temp_err() {
    if(!isset($_SESSION['temp_err']) OR ! $_SESSION['temp_err']) return FALSE;
    $msg = $_SESSION['temp_err'];
    unset($_SESSION['temp_err']);
    return $msg;
  }
  function _zen_show_temp_success() {
    if(!isset($_SESSION['temp_success']) OR ! $_SESSION['temp_success']) return FALSE;
    $msg = $_SESSION['temp_success'];
    unset($_SESSION['temp_success']);
    return $msg;
  }

  function _is_admin(){
    if(!empty($_SESSION['is_logged_in']) AND !empty($_SESSION['adminid'])) {
      return $_SESSION['adminid']; 
    }
    else return FALSE;
  }

  function _drive_date_format($date){
    return date('jS M (D), h:i A', strtotime($date));
  }
  
  function _readable_name($name){
    $titles = explode(' ',$name);
    foreach($titles as $title){
      $finalname = ucfirst($title);
      break;
    }
  ...
  function _selected_dropdown($selected, $value){//first one is the value passed, second one is the value of this particular option
  ...
  function _proper_name($name){
      if(strpos($name," ")){
        $titles = explode(' ', $name);
        $fullname = ucfirst($titles[0])." ".ucfirst($titles[1]);
        return $fullname;
      }
      return ucfirst($name);
  }
