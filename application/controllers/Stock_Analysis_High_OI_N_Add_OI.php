...
/*
 * @author: ZAHIR
 * DECS: Analysis of option chain by highest oi and addition of highest oi for both call and put
 */

class Stock_Analysis_High_OI_N_Add_OI extends MX_Controller {
    ...
    public function stockAnlaysisByHighestOiNAddOi(){
        
        $Send_Api_Contr = new Send_Api_Contr();
        
        $this->load->model('Analysis_task_model');
        
        $check_task_done = $this->Analysis_task_model->checkAnalysisDone('oc_high_oi_n_high_add_of_oi_analysis');
...
//        $this->load->model('Put_call_model'); #Delete after test
        ...
        $last_calculated_company = $this->Analysis_task_model->lastCalculatedCompany('oc_high_oi_n_high_add_of_oi_analysis');
...
//        echo '<pre>'; print_r($company_list); 
        ...
        if(empty($company_list)){            
            ...
            $this->Analysis_task_model->insertOcAnalysisDone('oc_high_oi_n_high_add_of_oi_analysis');
            echo '<br/>';
...
//            if ($underlying_date != '2019-12-18') {
            ...
            $this->Analysis_task_model->ocCalculationDone($company_id, $company_symbol, 'oc_high_oi_n_high_add_of_oi_analysis');#this means we have done this companies calulation for today
            ...
            $this->OcHighOiNHighAddOiCalculate( $company_id, $company_symbol, $underlying_date, $underlying_price, $expiry_dates );
...
//            exit(); # delete it after test
    ...
    /*
     * @author: ZAHIR
     * DESC: Calculation of option chain by highest oi and addition of highest oi for both call and put
     */
    
    function OcHighOiNHighAddOiCalculate( $company_id, $company_symbol, $underlying_date, $underlying_price, $expiry_dates ){
        
        echo '$underlying_price : ' . $underlying_price;
        echo '<br/>';
        
        $this->load->model('Oc_h_oi_n_h_addoi_analysis_model');
        ...
        foreach ($expiry_dates AS $expiry_dates_key=> $expiry_dates_value) {            
            ...
            echo '$expiry_date : <br/>';
            echo $expiry_dates_value->expiry_date;
            
            $high_call_oi = $this->Oc_h_oi_n_h_addoi_analysis_model->getHighestOi( $company_id, $company_symbol, $underlying_date, $expiry_dates_value->expiry_date, 'calls', 'calls_oi' );                        
            $high_call_add_oi = $this->Oc_h_oi_n_h_addoi_analysis_model->getHighestOi( $company_id, $company_symbol, $underlying_date, $expiry_dates_value->expiry_date, 'calls', 'calls_chng_in_oi' );                        
            
            echo '<br/>';
            
            $highest_oi_arr = array();
            
            if( !empty($high_call_oi) && !(empty($high_call_add_oi)) && !empty($high_call_oi[0]->strike_price) && !(empty($high_call_add_oi[0]->strike_price)) ){
                
                if( $high_call_oi[0]->strike_price === $high_call_add_oi[0]->strike_price ){
                    
                    echo 'highest oi and add oi found in sp(call): '. $high_call_oi[0]->strike_price;
                    echo '<br/>';
                    $highest_oi_arr['calls_oi'] = $high_call_oi[0]->calls_oi;
                    $highest_oi_arr['calls_chng_in_oi'] = $high_call_oi[0]->calls_chng_in_oi;
                    $highest_oi_arr['strike_price_in_call'] = $high_call_oi[0]->strike_price;
                    
                }
                
            }                        
            
            $high_puts_oi = $this->Oc_h_oi_n_h_addoi_analysis_model->getHighestOi( $company_id, $company_symbol, $underlying_date, $expiry_dates_value->expiry_date, 'puts', 'puts_oi' );                        
            $high_puts_add_oi = $this->Oc_h_oi_n_h_addoi_analysis_model->getHighestOi( $company_id, $company_symbol, $underlying_date, $expiry_dates_value->expiry_date, 'puts', 'puts_chng_in_oi' );                        
           
            if( !empty($high_puts_oi) && !(empty($high_puts_add_oi)) && !empty($high_puts_oi[0]->strike_price) && !(empty($high_puts_add_oi[0]->strike_price)) ){
                
                if( $high_puts_oi[0]->strike_price === $high_puts_add_oi[0]->strike_price ){
                    
                    echo 'highest oi and add oi found in sp(put): '. $high_puts_oi[0]->strike_price;
                    echo '<br/>';
                    $highest_oi_arr['puts_oi'] = $high_puts_oi[0]->puts_oi;
                    $highest_oi_arr['puts_chng_in_oi'] = $high_puts_oi[0]->puts_chng_in_oi;
                    $highest_oi_arr['strike_price_in_put'] = $high_puts_oi[0]->strike_price;
                    
                }
                
            }
            
            if(!empty($highest_oi_arr)){
                
                $highest_oi_arr['company_id'] = $company_id;
                $highest_oi_arr['company_symbol'] = $company_symbol;
                $highest_oi_arr['underlying_date'] = $underlying_date;
                $highest_oi_arr['underlying_price'] = $underlying_price;
                $highest_oi_arr['expiry_date'] = $expiry_dates_value->expiry_date;
                
                $highest_oi_arr["created_at"] = date("Y-m-d H:i:s");
            
                $this->Oc_h_oi_n_h_addoi_analysis_model->insertHighestOiData($highest_oi_arr);
            }
            
            
            echo '$highest_oi_arr : ';
            echo '<br/>';
            echo '<pre>'; print_r($highest_oi_arr);
    ...
    /*
     * @author: ZAHIR
     * DESC: calculation of option chain by highest oi and addition of highest oi for both call and put starting from first row mysql table
     */
    function ocHighOiNHighAddOiCalcForPrev() {
...
//            echo '<pre>'; print_r($allUnderlyingDate);
            ...
            foreach( $allUnderlyingDate AS $underlying_date_obj){
                ...
                $this->OcHighOiNHighAddOiCalculate( $company_id, $company_symbol, $underlying_date, $current_price, $expiry_dates );
